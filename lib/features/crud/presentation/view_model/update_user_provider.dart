...
class UpdateUserProvider extends ChangeNotifier {
  ...
  Future<void> updateUser() async {
    loading = true;
    notifyListeners();
    final response = await ref.watch(updateUserDataProvider).updateUserInfo(
        idCtr.text, nameCtr.text.toString(), jobCtr.text.toString());
    newUser = response;
    print(response.name);
    loading = false;
    notifyListeners();
...
final updateUserInfoProvider = ChangeNotifierProvider<UpdateUserProvider>(
    (ref) => UpdateUserProvider(ref: ref));
